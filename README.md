...
2. **修改配置**：编辑 `src/index.ts` 文件中的管理后台路由
3. **查找路由定义**：搜索 `adminPath` 相关配置
4. **修改默认路径**：将默认的管理路径改为您自定义的路径
5. **重新部署**：使用您修改后的代码进行部署

**示例配置**：
```typescript
// 在 src/index.ts 中找到类似的代码并修改
const defaultAdminPath = "/your-custom-admin-path"
```
